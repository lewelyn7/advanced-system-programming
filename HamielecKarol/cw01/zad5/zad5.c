...
void reader(int *i)
{
    ...
    while(1){
        ...
        if(libs[it].readers_count < LIB_MAX_READERS){
            break;
        }
    ...
    if (readers_count == 1)
    {
        ...
        sleep(1);
    pthread_mutex_unlock(&libs[it].r_mutex);
    printf("%sCzytelnik %d czytam z %d, jestem %d czytelnikiem....%s%s\r\n",TG, id, it, libs[it].readers_count, libs[it].books ,TRST);
    //czytania
